...
describe('CaliperService', function() {
	...
	describe('isInitialized async storage', function() {
		...
		it('returns true after resolving storage loads', function() {
	...
	describe('isInitialized with storage error', function() {
		...
		it('creates an empty array for the queue if there is an error reading from storage', function() {
			createService(store, options, function() {
				expect(store['studioKit:caliperService:queue'].length).to.equal(0);
			});
		});
