...
function CaliperService(id, options, getToken, storageService) {
	...
	function quizAssignableDigitalResource(id, quiz) {
		var nowISOString = moment().toISOString();
		...
		attempt.assignable = assignableDigitalResource;

		return {attempt: attempt, assignableDigitalResource: assignableDigitalResource};
	}
	...
	function startQuiz(id, quiz) {
		...
		let quizResources = quizAssignableDigitalResource(id, quiz);
		caliperQuiz(Caliper.Actions.AssessmentActions.STARTED, quizResources.assignableDigitalResource, quizResources.attempt, null, "start quiz");
	}

	function restartQuiz(id, quiz) {
		if (!softwareApplication || typeof softwareApplication === 'undefined') {
			throw new Error('Cannot restart quiz in Caliper: id is not defined. You must pass an id .');
		}

		let quizResources = quizAssignableDigitalResource(id, quiz);
		console.log('quiz resources are in restart quiz' + JSON.stringify(quizResources));

		caliperQuiz(Caliper.Actions.AssessmentActions.RESTARTED, quizResources.attempt, null, quizResources.assignableDigitalResource, "restart quiz");
	}

	function pauseQuiz(id, quiz) {
		if (!softwareApplication || typeof softwareApplication === 'undefined') {
			throw new Error('Cannot pause quiz in Caliper: id is not defined. You must pass an id .');
		}

		let quizResources = quizAssignableDigitalResource(id, quiz);
		console.log('quiz resources are in paused quiz ' + JSON.stringify(quizResources));

		caliperQuiz(Caliper.Actions.AssessmentActions.PAUSED, quizResources.attempt, null, quizResources.assignableDigitalResource, "pause quiz");
	}

	function endQuiz(id, quiz) {
		if (!softwareApplication || typeof softwareApplication === 'undefined') {
			throw new Error('Cannot end quiz in Caliper: id is not defined. You must pass an id .');
		}

		let quizResources = quizAssignableDigitalResource(id, quiz);
		console.log('quiz resources are ' + JSON.stringify(quizResources));
		caliperQuiz(Caliper.Actions.AssessmentActions.SUBMITTED, quizResources.attempt, null, quizResources.assignableDigitalResource, "end quiz");
	}

	// TODO: Better name
	function caliperQuiz(action, object, generated, target, label) {
		if (!isInitialized) {
