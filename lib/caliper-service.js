...
function CaliperService(options) {
	...
	function restartKeepAlive() {
		if (!_options.autoKeepAliveSession || _.isNil(session)) {
			return;
		}
	...
	function autoSend() {
		if (!_options.autoSend || !isInitialized) {
			return;
		}
		return send()
			.then(function() {
	...
	function onBeforeUnload() {
		if (!isInitialized || !_options.isEnabled() || _.isNil(session)) {
			return;
		}
		pauseSession();
	}
	...
	function endAndStartNewSession() {
