...
function CaliperService(options) {
	...
	function initialize() {
		...
		if (_options.autoEndBrowserSession) {
			addEndBrowserSessionListener();
		}
	...
	function trySend() {
		if (!isInitialized || sendPromise !== null) {
			setTimeout(trySend, _options.sendInterval);
			return;
		}
		sendPromise = service.send()
	...
	function addEndBrowserSessionListener() {
		...
		if (window.addEventListener) {
			window.addEventListener('beforeunload', tryEndSession);
		} else {
			window.onbeforeunload = tryEndSession;
		}
	}
