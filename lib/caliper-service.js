...
function CaliperService(id, options, getToken, storageService) {
	...
	function createAssessmentItem(id, isoString, options) {
		...
		if (options && _.isPlainObject(options) && Object.keys(options).length > 0) {
			Object.keys(options).map(key => {
				var value = options[key];
				if (!_.isNil(value))
					assessmentItem[key] = value;
			});
		}
	...
	function startAssessment(id, attemptId, options) {
		...
		if (_.isNil(attemptId)) {
			throw new Error('Cannot start Assessment in Caliper: attemptId is not defined. You must pass an attemptId .');
		}
	...
	function restartAssessment(id, attemptId) {
		...
		if (_.isNil(id)) {
			throw new Error('Cannot restart Assessment in Caliper: id is not defined. You must pass an id .');
		}
		if (_.isNil(attemptId)) {
			throw new Error('Cannot restart Assessment in Caliper: attemptId is not defined. You must pass an attemptId .');
		}
	...
	function pauseAssessment(id, attemptId) {
		...
		if (_.isNil(id)) {
			throw new Error('Cannot pause Assessment in Caliper: id is not defined. You must pass an id .');
		}
		if (_.isNil(attemptId)) {
			throw new Error('Cannot pause Assessment in Caliper: attemptId is not defined. You must pass an attemptId .');
		}
	...
	function endAssessment(id, attemptId) {
		...
		if (_.isNil(id)) {
			throw new Error('Cannot end Assessment in Caliper: id is not defined. You must pass an id .');
		}
		if (_.isNil(attemptId)) {
			throw new Error('Cannot end Assessment in Caliper: attemptId is not defined. You must pass an attemptId .');
		}
	...
	// function startAssessmentItem(id, attemptId, options) {
	// 	checkFullyInitialized();
	//
	// 	if (_.isNil(id)) {
	// 		throw new Error('Cannot start AssessmentItem in Caliper: id is not defined. You must pass an id .');
	// 	}
	// 	if (_.isNil(attemptId)) {
	// 		throw new Error('Cannot start AssessmentItem in Caliper: attemptId is not defined. You must pass an attemptId .');
	// 	}
	//
	// 	var nowISOString = moment().toISOString();
	// 	var assessmentItem = createAssessmentItem(id, nowISOString, options);
	// 	var attempt = createAttempt(attemptId, assessmentItem);
	//
	// 	enqueueAssessmentEvent(nowISOString, Caliper.Actions.AssessmentItemActions.STARTED, assessmentItem, attempt, null, "startAssessmentItem");
	// }
