...
function CaliperService(options) {
	...
	function validateOptions(options) {
		...
		if (!_.isNil(options.sessionIriPrefix) && !_.isString(options.sessionIriPrefix)) {
			throw new Error('`options.sessionIriPrefix` must be a string');
		}
		if (!_.isBoolean(options.autoKeepAliveSession)) {
			throw new Error('`options.autoKeepAliveSession` must be a boolean');
		}
		if (!_.isFinite(options.sessionKeepAliveInterval)) {
			throw new Error('`options.sessionKeepAliveInterval` must be a number');
		}
		if (!_.isFinite(options.sessionPauseThreshold)) {
			throw new Error('`options.sessionPauseThreshold` must be a number');
		}
		if (!_.isFunction(options.onError)) {
			throw new Error('`options.onError` function is required');
		}
		if (!_.isFunction(options.isEnabled)) {
			throw new Error('`options.isEnabled` function is required');
		}
		if (!_.isFunction(options.isGetTokenAuthorized)) {
			throw new Error('`options.isGetTokenAuthorized` function is required');
		}
	}
	...
	function checkIsGetTokenAuthorized() {
	...
	function tryGetOrRefreshAuthToken() {
		// do not check isEnabled or isGetTokenAuthorized
		// if we have a cached token or queue we want it to send
