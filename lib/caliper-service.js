...
function CaliperService(options) {
	...
	function loadSavedSession() {
		...
		if (moment().diff(moment(sessionPauseDate)) <= _options.sessionPauseThreshold) {
			...
			// persist person, session.extensions from previous session
			var extensionsCopy = session.extensions;
			var personCopy = person;
			endSession(sessionPauseDate);
			person = personCopy; // endSession clears the person, reset it
	...
	function endSession(endedAtTime) {
		...
		// clear session, person, and keepAlive
		clearSavedSession();
		clearSavedPerson();
