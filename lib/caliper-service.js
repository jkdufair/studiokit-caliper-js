...
function CaliperService(options, asyncLoadCallback) {
	...
	function initializeQueue() {
		var queueLoad = _options.storageService.getItem(queueKey);
		if (queueLoad && typeof queueLoad.then === 'function') {
			return Q.Promise(function(resolve, reject, notify) {
				queueLoad.then(function(resolvedQueue) {
					queue = resolvedQueue || [];
					resolve(resolvedQueue)
	...
	function resolveIfPromise(values, onComplete) {
		if (anyPromises(values)) {
			Q.allSettled(values).then(function(settledPromises) {
				var resolvedValues = settledPromises.map(function(value) { return value.value});
